...
# Emit the code
#---------------------------------------
asmfile = open("dft_amd64.s","w")

emitln("// DO NOT EDIT! This file generated by gen_amd64.jl")
...
#---------------------------------------
reset(16)
emitln("// func accumulateToFeet(z *[2]cvec, u *[2]u13, feet []foot)")
...
for i=1:2
    ...
    broadcast(bol(u1[i]), cmplxLoc(SI,0+16j))
    broadcast(bol(u3[i]), cmplxLoc(SI,8+16j))
end
mov(DI,Arg("feet_base",fpOff))
mov(CX,Arg("feet_len",fpOff+8))
...
for i=1:2
    t0 = Vreg()
    t1 = Vreg()
    mul(bol(t0), a[i].re, u1[i].re)
    mul(bol(t1), a[i].im, u1[i].re)
    add(fbc,eol(t1))
    add(fac,eol(t0))
end
...
for i=1:2
    t0 = Vreg()
    t1 = Vreg()
    mul(bol(t0), a[i].re, u1[i].im)
    mul(bol(t1), a[i].im, u1[i].im)
    add(fbd,eol(t1))
    add(fad,eol(t0))
end
...
#---------------------------------------
reset(8)
emitln("// func rotate( w []cvec, v []complex64)")
...
CX = Ireg("CX")
fpOff = 0
mov(DI,Arg("w_base",fpOff)); fpOff+=8
mov(CX,Arg("w_len",fpOff)); fpOff+=8
                        fpOff+=8
mov(SI,Arg("v_base",fpOff))
emitln("loop:")
v = Cmplx()
...
mov(CX,Arg("feet_len",fpOff)); fpOff+=8
